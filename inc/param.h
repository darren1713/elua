...
int32_t param_set_string( const char * name, const char * prefix, const char * value, int32_t len ); // returns bytes written for type (not including header)
int32_t param_get_string_len( const char * name, const char * prefix ); // returns byte length, <0 = error
int32_t param_get_string( const char * name, const char * prefix, char *value, uint32_t max_len ); // returns bytes read, <0 = error
