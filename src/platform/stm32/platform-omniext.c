...
#define HCLK        ( (HSE_Value/2) * 9 )
#define PCLK1_DIV   2
#define PCLK2_DIV   1
const uint32_t SystemCoreClock = HCLK;
...
void delay_us(volatile uint16_t msd)
{
    for(;msd;msd--) DELAY_9_CLK;
}

void delay_ms(volatile uint16_t msd)
{
    for(;msd;msd--) DELAY_1_MS;
}
...
int platform_init()
{
  ...
  TRACE("Am I running?\n");
  UART_SendChar('T', (ULONG)USART1);
  UART_SendChar('e', (ULONG)USART1);
  UART_SendChar('s', (ULONG)USART1);
  UART_SendChar('t', (ULONG)USART1);
  UART_SendChar('\n', (ULONG)USART1);
  ...
  // Enable SysTick
  if ( SysTick_Config( SystemCoreClock / SYSTICKHZ ) )
  { 
    /* Capture error */ 
    while (1);
  }
...
static void RCC_Configuration(void)
{
  ...
  //Setup PLL to multiply HSE of 16Mhz to 72Mhz --------------------
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1 | RCC_APB2Periph_GPIOA |
                         ...
                         RCC_APB2Periph_GPIOD | RCC_APB2Periph_AFIO | RCC_APB2Periph_SPI1 |
...
static GPIO_TypeDef * const pio_port[] = { GPIOA, GPIOB, GPIOC, GPIOD};
static const u32 pio_port_clk[]        = { RCC_APB2Periph_GPIOA, RCC_APB2Periph_GPIOB, RCC_APB2Periph_GPIOC, RCC_APB2Periph_GPIOD};
...
static void pios_init()
{
  ...
  // Disable the JTAG interface and enable the SWD interface
  ...
  //Omnima STM32Expander specific GPIO initialisation
  
  //LED, PWM or Audio out
  *GPIO_BB_ODR(GPIOB_BASE, 8)=0;
  ...
  {
      *GPIO_BB_ODR(GPIOB_BASE, 8)=~*GPIO_BB_ODR(GPIOB_BASE, 8);
      delay_ms(20);
  }
  ...
  //Reset - PD2 - Keep the Z-Wave module in reset until we're ready to 
  //		let it run
  *GPIO_BB_ODR(GPIOD_BASE, 2)=0;
...
//                                   SCK           MISO          MOSI
static GPIO_TypeDef *const spi_gpio_port[] = { GPIOA };
...
#define SPI_GET_BASE_CLK( id ) ( ( id ) == 0 ? ( SystemCoreClock / PCLK2_DIV ) : ( SystemCoreClock / PCLK1_DIV ) )
...
USART_TypeDef *const stm32_usart[] =          { USART1, USART2, USART3};
static GPIO_TypeDef *const usart_gpio_rx_port[] = { GPIOA, GPIOA, GPIOC };
static GPIO_TypeDef *const usart_gpio_tx_port[] = { GPIOA, GPIOA, GPIOC };
static const u16 usart_gpio_rx_pin[] = { GPIO_Pin_10, GPIO_Pin_3, GPIO_Pin_11 };
static const u16 usart_gpio_tx_pin[] = { GPIO_Pin_9, GPIO_Pin_2, GPIO_Pin_10 };
...
#define TIM_GET_BASE_CLK( id ) ( TIM_GET_PRESCALE( id ) == 1 ? ( SystemCoreClock / TIM_GET_PRESCALE( id ) ) : ( SystemCoreClock / ( TIM_GET_PRESCALE( id ) / 2 ) ) )
...
{
  const TIM_TypeDef* ptimer = timer[ id ];

  return TIM_GET_BASE_CLK( id ) / ( TIM_GetPrescaler( (TIM_TypeDef*)ptimer ) + 1 );
}
...
static u32 platform_timer_set_clock( unsigned id, u32 clock )
{
  ...
  TIM_Cmd( (TIM_TypeDef *)ptimer, ENABLE );
...
void platform_s_timer_delay( unsigned id, timer_data_type delay_us )
{
  ...
  TIM_SetCounter( (TIM_TypeDef *)ptimer, 0 );
  for( dummy = 0; dummy < 200; dummy ++ );
  while( TIM_GetCounter( (TIM_TypeDef *)ptimer ) < final );
}
...
timer_data_type platform_s_timer_op( unsigned id, int op, timer_data_type data )
{
  ...
  switch( op )
  {
    ...
    case PLATFORM_TIMER_OP_READ:
      res = TIM_GetCounter( (TIM_TypeDef *)ptimer );
      break;
...
void stm32_enc_init( unsigned id )
{
  ...
  TIM_Cmd( (TIM_TypeDef *)ptimer, DISABLE );
  TIM_DeInit( (TIM_TypeDef *)ptimer );
  TIM_SetCounter( (TIM_TypeDef *)ptimer, 0 );
  TIM_EncoderInterfaceConfig( (TIM_TypeDef *)ptimer, TIM_EncoderMode_TI12, TIM_ICPolarity_Rising, TIM_ICPolarity_Rising);
  TIM_Cmd( (TIM_TypeDef *)ptimer, ENABLE );
}
...
void stm32_enc_set_counter( unsigned id, unsigned count )
{
  ...
  TIM_SetCounter( (TIM_TypeDef *)ptimer, ( u16 )count );
}
...
extern const LUA_REG_TYPE w1_map[];
extern const LUA_REG_TYPE omniexp_map[];

const LUA_REG_TYPE platform_map[] =
{
#if LUA_OPTIMIZE_MEMORY > 0
  { LSTRKEY( "enc" ), LROVAL( enc_map ) },
  { LSTRKEY( "w1" ),  LROVAL( w1_map ) },
  { LSTRKEY( "omniexp" ), LROVAL( omniexp_map ) },
#endif
  { LNILKEY, LNILVAL }
};
...
LUALIB_API int luaopen_platform( lua_State *L )
{
  ...
  lua_newtable( L );
  luaL_register( L, NULL, w1_map );
  lua_setfield( L, -2, "w1" );  
  
  lua_newtable( L );
  luaL_register( L, NULL, omniexp_map );
  lua_setfield( L, -2, "omniexp" );    
