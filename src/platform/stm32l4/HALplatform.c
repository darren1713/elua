...
static void pios_init()
{
  ...
  for( port = 0; port < NUM_PIO; port++ )
  {
    ...
    HAL_GPIO_Init(pio_port[port], &GPIO_InitStructure);
  }

#if defined(ENABLE_JTAG_SWD) || defined(ENABLE_TRACE)
  //Mapping JTAG / SWD pins
  GPIO_PINAFConfig(GPIOB, GPIO_PIN_4,  GPIO_AF0_SWJ); // PB4  TRST
  GPIO_PINAFConfig(GPIOB, GPIO_PIN_3,  GPIO_AF0_SWJ); // PB3  TDO   / SWO

  GPIO_PINAFConfig(GPIOA, GPIO_PIN_13, GPIO_AF0_SWJ); // PA13 TMS   / SWDIO
  GPIO_PINAFConfig(GPIOA, GPIO_PIN_14, GPIO_AF0_SWJ); // PA14 TCK   / SWDCLK
  GPIO_PINAFConfig(GPIOA, GPIO_PIN_15, GPIO_AF0_SWJ); // PA15 TDI
  ...
  GPIO_PINAFConfig(GPIOE, GPIO_PIN_2, GPIO_AF_TRACE);
  GPIO_PINAFConfig(GPIOE, GPIO_PIN_3, GPIO_AF_TRACE);
  GPIO_PINAFConfig(GPIOE, GPIO_PIN_4, GPIO_AF_TRACE);
  GPIO_PINAFConfig(GPIOE, GPIO_PIN_5, GPIO_AF_TRACE);
  GPIO_PINAFConfig(GPIOE, GPIO_PIN_6, GPIO_AF_TRACE);
#endif
...
//#if defined( ELUA_BOARD_STM32L4ALT )
  ...
  static const u8 usart_gpio_rx_pin_source[] = { GPIO_PIN_10, GPIO_PIN_3, GPIO_PIN_11, GPIO_PIN_11, GPIO_PIN_2 };
  static const u16 usart_gpio_tx_pin[] = { GPIO_PIN_9, GPIO_PIN_2, GPIO_PIN_10, GPIO_PIN_10, GPIO_PIN_12 };
  static const u8 usart_gpio_tx_pin_source[] = { GPIO_PIN_9, GPIO_PIN_2, GPIO_PIN_10, GPIO_PIN_10, GPIO_PIN_12 };
//#else
...
//  static const u8 usart_gpio_rx_pin_source[] = { GPIO_PIN_7, GPIO_PIN_3, GPIO_PIN_11, GPIO_PIN_11, GPIO_PIN_2 };
//  static const u16 usart_gpio_tx_pin[] = { GPIO_PIN_6, GPIO_PIN_2, GPIO_PIN_10, GPIO_PIN_10, GPIO_PIN_12 };
//  static const u8 usart_gpio_tx_pin_source[] = { GPIO_PIN_6, GPIO_PIN_2, GPIO_PIN_10, GPIO_PIN_10, GPIO_PIN_12 };
//#endif
...
static void usart_init(u32 id, USART_InitTypeDef * initVals)
{
  ...
  /* Enable USART */
  __HAL_USART_ENABLE(stm32_usart[id]);
}
...
#endif  
  {
    while(__HAL_USART_GET_FLAG(stm32_usart[id], USART_FLAG_TXE) == RESET);
...
int platform_s_uart_recv( unsigned id, timer_data_type timeout )
{
  ...
  if( timeout == 0 )
  {
    if (__HAL_USART_GET_FLAG(stm32_usart[id], USART_FLAG_RXNE) == RESET)
      return -1;
    else
      return USART_ReceiveData(stm32_usart[id]);
  }
  // Receive char blocking
  while(__HAL_USART_GET_FLAG(stm32_usart[id], USART_FLAG_RXNE) == RESET);
  return USART_ReceiveData(stm32_usart[id]);
}
...
static const u8 can_gpio_rx_pin_source[] =   { GPIO_PIN_0,     GPIO_PIN_12 };
static const u16 can_gpio_tx_pin[] =         { GPIO_PIN_1,          GPIO_PIN_13 };
static const u8 can_gpio_tx_pin_source[] =   { GPIO_PIN_1,     GPIO_PIN_13 };

#elif defined( ELUA_BOARD_STM32F4_CAN_PIN_CONFIG_2 )

static GPIO_TypeDef *const can_gpio_port[] = { GPIOD,               GPIOB };
static const u16 can_gpio_rx_pin[] =         { GPIO_PIN_0,          GPIO_PIN_5 };
static const u8 can_gpio_rx_pin_source[] =   { GPIO_PIN_0,     GPIO_PIN_5 };
static const u16 can_gpio_tx_pin[] =         { GPIO_PIN_1,          GPIO_PIN_6 };
static const u8 can_gpio_tx_pin_source[] =   { GPIO_PIN_1,     GPIO_PIN_6 };
...
static const u8 can_gpio_rx_pin_source[] =   { GPIO_PIN_8,     GPIO_PIN_12 };
static const u16 can_gpio_tx_pin[] =         { GPIO_PIN_9,          GPIO_PIN_13 };
static const u8 can_gpio_tx_pin_source[] =   { GPIO_PIN_9,     GPIO_PIN_13 };

#elif defined( ELUA_BOARD_STM32F4_CAN_PIN_CONFIG_4 )

static GPIO_TypeDef *const can_gpio_port[] = { GPIOB,               GPIOB };
static const u16 can_gpio_rx_pin[] =         { GPIO_PIN_8,          GPIO_PIN_5 };
static const u8 can_gpio_rx_pin_source[] =   { GPIO_PIN_8,     GPIO_PIN_5 };
static const u16 can_gpio_tx_pin[] =         { GPIO_PIN_9,          GPIO_PIN_6 };
static const u8 can_gpio_tx_pin_source[] =   { GPIO_PIN_9,     GPIO_PIN_6 };
...
static const u8 pwm_GPIO_PINs_source[] = { GPIO_PIN_6, GPIO_PIN_7, GPIO_PIN_8, GPIO_PIN_9 };
#else
...
static const u8 pwm_GPIO_PINs_source[] = { GPIO_PIN_12, GPIO_PIN_13, GPIO_PIN_14, GPIO_PIN_15 };
#endif
...
u32 platform_pwm_setup( unsigned id, u32 frequency, unsigned duty )
{
  ...
  TIM_OCInitStructure.Pulse = ( u16 )( duty * ( PWM_TIMER_NAME->ARR + 1 ) / 100 );
  TIM_OCInitStructure.OCPolarity = TIM_OCPOLARITY_HIGH;
...
u32 platform_adc_set_clock( unsigned id, u32 frequency )
{
  ...
  if ( frequency > 0 )
  {
    ...
    timer_base_struct.Period = period - 1;
    timer_base_struct.Prescaler = prescaler - 1;
    timer_base_struct.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    timer_base_struct.CounterMode = TIM_COUNTERMODE_DOWN;
